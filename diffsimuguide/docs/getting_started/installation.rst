...
Install from github with pip::
    ...
    pip install -e git+https://github.com/Bradley-Karat, mjallais/diffsimuguide#egg=diffsimuguide
...
Do a dry-run first (``-n``) and simply print (``-p``) what would be run::
    ...
    diffsimuguide /path/to/bids/dir /path/to/output/dir participant -np

Run the app, using all cores::
    
    diffsimuguide /path/to/bids/dir /path/to/output/dir participant --cores all
...
To generate a report, run::
    ...
    diffsimuguide /path/to/bids/dir /path/to/output/dir participant --report
...
Here are some instructions to get your python environment set-up on graham to run diffsimuguide:

#. Create a virtualenv and activate it::

      cd $SCRATCH
      module load python/3
      virtualenv venv_diffsimuguide
      source venv_diffsimuguide/bin/activate
...
Scaling up to even more subjects (uses group-components to bundle multiple subjects in each job), 1 32core job for N subjects (e.g. 10)::
    ...
    diffsimuguide bids_dir out_dir participant  --profile cc-slurm --group-components subj=10
